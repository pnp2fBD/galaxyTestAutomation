...
public class ExtentReporter {
  ...
  private static File configFile =
      new File(System.getProperty("user.dir") + File.separator + "ReportConfig.xml");
  ...
  /**
   * Returns an ExtentReports instance if already exists. Creates new and returns otherwise.
   * 
   * @param iTestResult
  ...
  private static synchronized ExtentReports getReportInstance(ITestResult iTestResult) {
    if (report == null) {
      String reportFilePath =
          new File(iTestResult.getTestContext().getOutputDirectory()).getParent() + File.separator
              + "ExtentReport.html";
  ...
  /**
   * To start and return a new extent test instance with given test case description. Returns the
   * test instance if the test has already been started
   * 
  ...
  private static ExtentTest startTest(String description) {
    ...
    String testName =
        iTestResult != null ? getTestName(iTestResult) : Thread.currentThread().getName();
    Integer hashCode =
        iTestResult != null ? iTestResult.hashCode() : Thread.currentThread().hashCode();
  ...
  /**
   * Returns the test instance if the test has already been started. Else creates a new test with
   * empty description
   * 
   * @return {@link ExtentTest} - ExtentTest Instance
   */
  ...
  public static void testCaseInfo(String testCaseInfo) {
    startTest(
        "<strong><font size = \"4\" color = \"#000080\">" + testCaseInfo + "</font></strong>");
  }
  ...
  public static void logStackTrace(Throwable t) {
    if (t instanceof SkipException) {
      getTest().log(LogStatus.SKIP,
          "<div class=\"stacktrace\">" + ExceptionUtils.getStackTrace(t) + "</div>");
    } else {
